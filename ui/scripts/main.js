const timeObjectToSeconds = (time) => (time.hours * 60 * 60) + (time.minutes * 60) + time.seconds;
...
const convertToSeconds = (timeString) => timeObjectToSeconds(getTimeAsObject(timeString));
...
const onClipButtonClick = async () => {
    ...
    try {
        ...
        if (!isTimestampWithinDuration(from, duration) || !isTimestampWithinDuration(to, duration)) {
            toastr.error('Please use timestamps that are within the video\'s duration.', 'Invalid Timestamps');
            return;
        }
